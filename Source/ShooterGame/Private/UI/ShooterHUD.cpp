...
AShooterHUD::AShooterHUD(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{
	...
	static ConstructorHelpers::FObjectFinder<UTexture2D> OrangeHighPingOb(TEXT("/Game/UI/HUD/orange_high_ping"));
	static ConstructorHelpers::FObjectFinder<UTexture2D> RedHighPingOb(TEXT("/Game/UI/HUD/red_high_ping"));
	static ConstructorHelpers::FObjectFinder<UTexture2D> RedUnstablePingOb(TEXT("/Game/UI/HUD/red_unstable_ping"));
	...
	OrangeHighPing = OrangeHighPingOb.Object;
	RedHighPing = RedHighPingOb.Object;
	RedUnstablePing = RedUnstablePingOb.Object;

	OrangeHighPingIndicator = UCanvas::MakeIcon(OrangeHighPing, 64, 64, 64, 64);
	RedHighPingIndicator = UCanvas::MakeIcon(RedHighPing, 64, 64, 64, 64);
	RedUnstablePingIndicator = UCanvas::MakeIcon(RedUnstablePing, 64, 64, 64, 64);
...
double AShooterHUD::avg(TArray<double> &pings) {
	double tmp = 0.0;
	for (int i = 0; i < 5; ++i) {
		tmp += pings[i];
	}
...
void AShooterHUD::storeNewPing(TArray<double> &pings, double newPing) {
	pings.RemoveAt(0);
	pings.Add(newPing);
}

void AShooterHUD::DrawIndicator() {
	static double avgPing = 0.0;
	const FVector2D ViewportSize = FVector2D(GEngine->GameViewport->Viewport->GetSizeXY());
	const float IndicatorPosX = ViewportSize.X - 64;
	const float IndicatorPosY = 64;
	const float IndicatorPosYUnstable = IndicatorPosY + 64;
	...
	double ping = (MyPlayerState->Ping) * 4;

	// Compare avg ping with newly received ping here.
	// Ouput Image if necessary.

	avgPing = avg(recentPings);
	if (abs(avgPing - ping) >= 20) {
		// Display unstable ping icon.
		Canvas->SetDrawColor(FColor::Orange);
		Canvas->DrawIcon(RedUnstablePingIndicator, IndicatorPosX, IndicatorPosYUnstable, ScaleUI);
	}

	storeNewPing(recentPings, ping);

	if (ping >= 200 && ping < 300) {
		Canvas->SetDrawColor(FColor::Orange);
		Canvas->DrawIcon(OrangeHighPingIndicator, IndicatorPosX, IndicatorPosY, ScaleUI);
	}
	else if (ping >= 300) {
		Canvas->SetDrawColor(FColor::Red);
		Canvas->DrawIcon(RedHighPingIndicator, IndicatorPosX, IndicatorPosY, ScaleUI);
	}
...
void AShooterHUD::DrawKills()
{
	...
	if (MyPlayerState)
	{
		Text = FString::FromInt(MyPlayerState->Ping * 4); // LLLLLLLLLLLLLLLLLLLLL GetKills()
	}
	else
	{
		Text = FString("0");
