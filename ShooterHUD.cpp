...
AShooterHUD::AShooterHUD(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{
	...
	//added locate images
	static ConstructorHelpers::FObjectFinder<UTexture2D> IconOneOb(TEXT("/Game/UI/HUD/orange_high_ping"));
	static ConstructorHelpers::FObjectFinder<UTexture2D> IconTwoOb(TEXT("/Game/UI/HUD/red_high_ping"));
	static ConstructorHelpers::FObjectFinder<UTexture2D> IconPacketLossOb(TEXT("/Game/UI/HUD/red_packet_loss"));
	...
	//added set the objects of indicators
	IconOne = IconOneOb.Object;
	IconTwo = IconTwoOb.Object;
	IconPacketLoss = IconPacketLossOb.Object;

	//added set the dimensions of indicators
	IconO = UCanvas::MakeIcon(IconOne, 64, 64, 64, 64);
	IconT = UCanvas::MakeIcon(IconTwo, 64, 64, 64, 64);
	IconPL = UCanvas::MakeIcon(IconPacketLoss, 64, 64, 64, 64);
...
//added
void AShooterHUD::DrawPacketLoss() {
	const FVector2D ViewportSize = FVector2D(GEngine->GameViewport->Viewport->GetSizeXY());
	AShooterPlayerController* MyPC = Cast<AShooterPlayerController>(PlayerOwner);
	UNetConnection* MyConnection = Cast<UNetConnection>(MyPC->NetConnection);
	const float IndicatorPosX = ViewportSize.X - 129;
	const float IndicatorPosY = 64;
	
	if (MyConnection->OutPacketsLost >= 10)
	{
		Canvas->SetDrawColor(FColor::Red);
		Canvas->DrawIcon(IconPL, IndicatorPosX, IndicatorPosY, ScaleUI);
	}
}

//added function for drawing indicators
void AShooterHUD::DrawIndicator() {
	...
	const float IndicatorPosY = 64;
	if (MyPlayerState->Ping * 4 >= 200 && MyPlayerState->Ping * 4 < 300) {
		Canvas->SetDrawColor(FColor::Orange);
		Canvas->DrawIcon(IconO, IndicatorPosX, IndicatorPosY, ScaleUI);
	}
	else if (MyPlayerState->Ping * 4 >= 300) {
		Canvas->SetDrawColor(FColor::Red);
		Canvas->DrawIcon(IconT, IndicatorPosX, IndicatorPosY, ScaleUI);
	}
...
void AShooterHUD::DrawKills()
{
	...
	if (MyPlayerState)
	{
		Text = FString::FromInt(MyPlayerState->Ping * 4); // fix back to MyPlayerState->Ping LLLLLLLLLLLLLLLLLLLLL GetKills()
	}
	else 
	{
		Text = FString("0");
...
void AShooterHUD::DrawHUD()
{
	...
	if (MatchState == EShooterMatchState::Playing)
	{
		...
		if (MyPawn && MyPawn->IsAlive())
		{
			...
			//added call the draw indicator function
			DrawIndicator();
			DrawPacketLoss();
		}
		else
